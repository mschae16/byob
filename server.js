  ...
  if (!token) {
    ...
    jwt.verify(token, app.get('secretKey'), (err, decoded) => {
      ...
      if (decoded) {
        ...
        return response.status(403).json({
          error: 'You are not authorized to have write access to this endpoint.'
        })
      }
    });
  };
...
app.post('/api/v1/user/authenticate', (request, response) => {
  ...
  if (!email || !app_name) {
    return response.status(422).json({
      error: 'You are missing a required parameter. Please include both email address and the name of your application.'
    });
  }
  const emailSuffix = email.split('@')[1];
  emailSuffix === 'turing.io' ?
    user = Object.assign({}, request.body, {
      admin: true
    }) :
    user = Object.assign({}, request.body, {
      admin: false
...
app.get('/api/v1/ports', checkToken, (request, response) => {
  database('ports').select()
    .then(ports => {
      ...
      ports.forEach(port => {
        portPromises.push(
          database('port_usage').where({
            port_id: port.id
          }).select()
          .then(usage => Object.assign({}, port, {
            port_usage: usage[0]
          }))
        )
      })
  ...
  if (Object.keys(request.query).length) {
    if (name) {
      ...
      queryObject = Object.assign({}, {
        ship_name: alteredName.toUpperCase()
      });
    }
    if (port) {
      queryObject = Object.assign({}, queryObject, {
        ship_current_port: port
      });
    }
  }
...
app.get('/api/v1/ports/:id', checkToken, (request, response) => {
  ...
  database('ports').where({
      id
    }).select()
    .then(port => {
      if (!port.length) {
        return response.status(404).json({
          error: 'There is no port with this id.'
        })
      } else {
        const portPromise = [];
        ...
        portPromise.push(
          ...
          .then(usage => Object.assign({}, port[0], {
            port_usage: usage[0]
          }))
          .catch(error => response.status(500).json({
            error
...
app.get('/api/v1/ships/:id', checkToken, (request, response) => {
  ...
  database('ships').where({
      id
    }).select()
    .then(ship => {
      if (!ship.length) {
        return response.status(404).json({
          error: 'There is no ship with this id.'
        })
      }
  ...
  for (let requiredParameter of [
    ...
    if (!portObject[requiredParameter]) {
      return response
        .status(422)
        .send({
          error: `Expected format: { port_name: <String>, port_locode: <String>, port_max_vessel_size: <String>, port_total_ships: <Integer>, port_country: <String>, port_usage: <Object> }. You're missing a ${requiredParameter} property.`
        });
    }
  };
  ...
  database('ports').insert({
    ...
    .then(port => {
      ...
      database('port_usage').insert(
          Object.assign({}, portObject.port_usage, {
            port_id: port[0].id
          }), '*')
        .then(result => {
          return response.status(201).json(Object.assign({}, port[0], {
            port_usage: result[0]
          }))
        })
        .catch(error => response.status(500).json({
  ...
  for (let requiredParameter of [
    ...
    if (!shipObject[requiredParameter]) {
      return response
        .status(422)
        .send({
          error: `Expected format: { ship_name: <String>, ship_country: <String>, ship_type: <String>, ship_length: <String>, ship_imo: <String>, ship_status: <String>, ship_mmsi_callsign: <String>, ship_current_port: <Integer> }. You're missing a ${requiredParameter} property.`
        });
    }
  }
...
app.delete('/api/v1/ships/:id', checkAuth, (request, response) => {
  ...
  database('ships').where({
    ...
    .then(deleted => {
      if (!deleted) {
        return response.status(404).json({
          error: 'A ship matching the id submitted could not be found.'
        })
      }
      return response.sendStatus(204)
    })
...
app.put('/api/v1/port-usage/:id', checkAuth, (request, response) => {
  ...
  if (request.body.port_id) {
    return response
      .status(422)
      .send({
        error: 'You are not authorized to change the port id'
      });
  }
